...
public class StoryCreator {
	...
	public static ContentValues getCVfromStory(final StoryData data) {
		...
		rValue.put(MoocSchema.Story.Cols.KEY, data.key);
		rValue.put(MoocSchema.Story.Cols.HREF, data.href);
		return rValue;
	}
	...
	public static StoryData getStoryDataFromCursor(Cursor cursor) {
		...
		String key = cursor.getString(cursor
				.getColumnIndex(MoocSchema.Story.Cols.KEY));

		String href = cursor.getString(cursor
				.getColumnIndex(MoocSchema.Story.Cols.HREF));

		// construct the returned object
		StoryData rValue = new StoryData(key, href, rowID, loginId, storyId,
				title, body, audioLink, videoLink, imageName, imageMetaData,
