...
public class MoocProvider extends ContentProvider {
    ...
    // LoggerFactory.getLogger("mooc.provider");

    // Local backend DB
    MoocDataDBAdaptor mDB;
    ...
    synchronized public boolean onCreate() {
        ...
        mDB = new MoocDataDBAdaptor(getContext());
        mDB.open();
        return true;
    }
    ...
    public ParcelFileDescriptor openFile(Uri uri, String mode) {
        ...
        case MoocSchema.Story.PATH_FOR_ID_TOKEN: {
            ...
            // Integer.toHexString(imode), storyFile, uri);
            try {
                if (!storyFile.exists()) {
                ...
                // logger.error("could not open file {}", storyFile, ex);
            } catch (IOException ex) {
                // logger.error("could not create file {}", storyFile, ex);
            }
        }
        case MoocSchema.Tags.PATH_FOR_ID_TOKEN: {
            ...
            final File tagsFile = new File(imageDirectory, "tags"
                    + segments.get(1));
            // logger.info("image file mode= path=, uri=",
            // Integer.toHexString(imode), tagsFile, uri);
            try {
                if (!tagsFile.exists()) {
                ...
                // logger.error("could not open file {}", tagsFile, ex);
            } catch (IOException ex) {
                // logger.error("could not create file {}", tagsFile, ex);
            }
        }
            break;
        default: {
